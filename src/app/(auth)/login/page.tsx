...
function LoginPageInner() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(response.status === 429 || response.status === 503 ? "Too many requests. Please try again later." : GENERIC_AUTH_ERROR);
        return;
      }
      const destination = inviteToken ? `/join/${encodeURIComponent(inviteToken.slice(0, 256))}` : "/dashboard";
